...
def test_create_user_should_create(decode_mock, delete_user_mock, client):
    # given
    ...
    # when
    response = client.delete("/user", headers={"Accept": "text/plain", "Authorization": "test"})
...
@mock.patch("application.user.user_rest_adapter.jwt.decode")
def test_create_user_wrong_header(decode_mock, client):
    # given
    error = WrongHeaderError("test")
    decode_mock.return_value = JWTPayload("test", "test", 0, 0, UUID("be964b8c-59cf-4549-9f25-eb6bb63824ec"))

    # when
    response = client.delete("/user", headers={"Accept": "application/ogg", "Authorization": "test"})
...
@mock.patch("application.user.user_rest_adapter.modify.delete_user")
@mock.patch("application.user.user_rest_adapter.jwt.decode")
def test_create_user_wrong_authorization(decode_mock, delete_user_mock, client):
    # given
...
@mock.patch("application.user.user_rest_adapter.modify.delete_user")
@mock.patch("application.user.user_rest_adapter.jwt.decode")
def test_create_user_no_authorization(decode_mock, delete_user_mock, client):
    # given
